...
- Webpack 적용 : react-script를 사용하지 않고 Webpack 직접 설정
- 칼럼 확장 : 기존 1개의 칼럼에서 4개로 확장
- 드래그 제약 조건
  - 1번째 칼럼에서 3번쨰 칼럼으로 이동 제한
  - 짝수 아이템 인접 여부 확인
  - 이동 불가 아이템 이동 시 빨간색으로 색상 변경 및 이동 제한
- 다중 선택 구현 : cmd + 클릭으로 다중 선택 및 이동 기능 구현
- 아이템 추가 기능
- 아이템 삭제 기능
